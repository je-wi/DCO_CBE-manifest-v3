...
var content2show;
  ...
  browser.storage.local.get(null, function(data)
    {  
      /* clone the document for manipulating the orginal document */
      ...
      */
      ...
      /* some central images from block plugin had relative pathes - maybe deprecated*/
      addLeadingHost(document,data.option2);
    
      /* in the archive site every issue has its on div*/
      var all_issues = doc.querySelectorAll('.obj_issue_summary'); 
      for(var i=0;i<all_issues.length;i++)
        ...
        /* extract some informations from the issue summary div */
        var titleElement = issue.querySelector('.title');
        var href = titleElement.getAttribute('href').trim();
        var title = titleElement.innerHTML.trim();
        var issue_id = 'issue_id_'+href.replace(data.option2+'/index.php/'+data.option3+'/issue/view/',''); 
        var coverSrc = issue.querySelector('.cover img').getAttribute('src').trim();
        ...
        var urlCoverElement = document.createElement("url");
        urlCoverElement.setAttribute('type','cover');
        urlCoverElement.innerHTML=coverSrc;
        biblElement.appendChild(urlCoverElement);        
        ...
        if(request.status==200)
          {
          /* response from issue/view  is a html-site*/
          var doc_issue = new DOMParser().parseFromString(request.responseText, 'text/html');
          
          /* there ar some warning boxes in ojs3 to use the latest article-version for citation not the complete issue-version */
          doc_issue.querySelectorAll(".cmp_notification.warning").forEach(el => el.remove());
            ...
            for(var sas=0; sas<section_articles.length;sas++)
              ...
              doRequest(viewurl, 'GET', function(ArticleRequest) {
                ...
                for(var gli=0; gli<galleys_links.length; gli++)
                  ...
                  // we have to parse the view as html to get the correct download url for pdf-galley from javascript in the body 
                  if( galley_type=="PDF" )
                    {
                    doRequest(galley_href, 'GET', function(PDFRequest) {
                      var pdf_html = new DOMParser().parseFromString(PDFRequest.responseText, 'text/html');
                      var jsscripts = pdf_html.querySelectorAll('body script');
                      for(var jsi=0; jsi<jsscripts.length; jsi++)
                        {
                        var node = jsscripts[jsi];
                        var str0 = node.innerText.search(/pdfUrl = "/);
                        if(str0!=-1)
                          {
                          var sub_string = node.innerText.substr(str0+10);
                          var pdf_download_url = sub_string.substr( 0, sub_string.indexOf('"') );
                          pdf_download_url = pdf_download_url.replaceAll(/\\\//g, "/");
                          //console.log(pdf_download_url);
                          
                          var urlElement2 = document.createElement("url");
                          urlElement2.setAttribute('type','downloadPDF');
                          urlElement2.innerHTML = pdf_download_url;
